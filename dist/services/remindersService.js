...
import { computeNextOccurrence } from "../lib/parseWhen.js";
export async function createReminder(accountId, input) {
    try {
...
export async function fetchDueReminders(limit = 50) {
    try {
        ...
        const { data, error } = await supabaseAdmin
            ...
            .eq("status", "pending")
            .or(`and(remind_at.lte.${new Date().toISOString()},delivery_attempts.lt.${MAX_DELIVERY_ATTEMPTS}),` +
...
export async function markReminderSent(reminderId, accountId, message, recurrenceRule, remindAtIso) {
    try {
...
export async function listPendingReminders(accountId, limit = 15) {
    try {
        const { data, error } = await supabaseAdmin
            .from("reminders")
...
export async function snoozeReminder(accountId, reminderId, delayMs) {
    try {
        ...
        if (lookupError)
            throw lookupError;
        if (!existing)
            return { ok: false, error: "Reminder not found", code: "not_found" };
...
export async function restoreReminderTime(accountId, reminderId, remindAtIso) {
    try {
...
export async function deleteReminder(accountId, reminderId) {
    try {
...
export async function markReminderFailedAttempt(reminderId, attempts, errorMessage) {
    try {
...
export async function markAlarmDelivered(reminderId) {
    try {
        await supabaseAdmin
            .from("reminders")
...
export async function acknowledgeReminder(accountId, reminderId) {
    try {
        const { error } = await supabaseAdmin
            .from("reminders")
