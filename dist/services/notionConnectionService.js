...
export async function issueOAuthState(accountId) {
    try {
...
export async function consumeOAuthState(state) {
    try {
        ...
        if (error)
            throw error;
        const match = (candidates ?? []).find((row) => crypto.timingSafeEqual(Buffer.from(row.state_hash), Buffer.from(stateHash)));
        if (!match)
            return { ok: false, error: "This connection link is invalid or has expired", code: "expired" };
 ...
 * row. */
export async function saveNotionConnection(accountId, workspaceId, workspaceName, accessToken) {
    try {
 ...
 * value. */
export async function getNotionConnection(accountId) {
    try {
...
export async function getNotionConnectionByWorkspace(workspaceId) {
    try {
        const { data, error } = await supabaseAdmin
            .from("notion_connections")
            .select("id, account_id, workspace_id, workspace_name, access_token_encrypted, access_token_iv, access_token_auth_tag, database_id")
...
export async function setNotionDatabaseId(accountId, databaseId) {
    try {
...
export async function disconnectNotion(accountId) {
    try {
...
export async function recordWebhookEventOnce(eventId) {
    try {
...
//# sourceMappingURL=notionConnectionService.js.map