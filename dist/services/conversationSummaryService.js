 ...
 */
export async function summarizeConversation(accountId, exchanges) {
    if (!groqClient)
    ...
    try {
        ...
        const completion = await groqClient.chat.completions.create({
            ...
            messages: [
                {
                    role: "system",
                    content: "Summarize the following conversation in 2-4 concise sentences. " +
...
export async function maybeSummarizeOldConversation(accountId, totalExchanges) {
    if (!isGroqConfigured)
        return;
    if (!checkRateLimit(`groq-summary:${accountId}`, env.GROQ_MAX_CALLS_PER_HOUR, 60 * 60 * 1000))
        return;
    // Only summarize when we cross the threshold (not every single time)
    if (totalExchanges < SUMMARIZATION_THRESHOLD)
        return;
    if (totalExchanges % SUMMARIZATION_THRESHOLD !== 1)
        return;
    const oldExchanges = await fetchExchangesForSummarization(accountId, SUMMARIZATION_BATCH);
    if (oldExchanges.length < 3)
        return;
...
//# sourceMappingURL=conversationSummaryService.js.map