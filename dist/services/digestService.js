...
export async function fetchAccountsDueForDigest() {
    try {
...
export async function markDigestSentToday(accountId, timeZone) {
    try {
...
export async function buildDigestContent(accountId, timeZone) {
    try {
        const { dateKey } = getLocalHourAndDateKey(timeZone);
...
export function formatDigestMessage(content) {
    const lines = ["☀ Your daily digest"];
    ...
    const hasAnyContent = content.tasksDueTodayOrOverdue.length > 0 || content.remindersToday.length > 0 || content.recentNotes.length > 0;
    if (!hasAnyContent) {
