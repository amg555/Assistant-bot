...
`);
    ...
    if (notionEncryptionKey)
        console.log(`  ${dim("Notion encryption key")}     ${dim("(auto-generated)")}`);
    const envLines = [
        `NODE_ENV=production`,
        `PORT=10000`,
        `ALLOWED_ORIGINS=*`,
    ...
    if (doMigration)
        console.log(green("  ✓ Database migration run"));
    if (doWebhook)
        console.log(green("  ✓ Telegram webhook registered"));
    if (doDiscordCommands)
        console.log(green("  ✓ Discord commands registered"));
    console.log(`
${bold("Next steps:")}
