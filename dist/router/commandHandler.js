import { resolveOrCreateAccount, issueLinkCode, consumeLinkCode, isAiEnabledForAccount, setAiEnabledForAccount, getAccountTimeZone, setAccountTimeZone, setDigestEnabled, getOrCreateWebhookSecret, } from "../services/accountService.js";
...
import { createReminder, listPendingReminders, snoozeReminder, restoreReminderTime, deleteReminder, acknowledgeReminder, } from "../services/remindersService.js";
...
import { createNoteSchema, createTaskSchema, createReminderSchema, linkCodeConsumeSchema, chartRequestSchema, setTimezoneSchema, setDigestSchema, snoozeReminderSchema, safeValidate, } from "../validation/schemas.js";
...
export async function handleCommand(cmd) {
    ...
    try {
        ...
        if (lower.startsWith("note ")) {
            ...
            if (!validation.ok)
                return { kind: "text", text: `Hmm, couldn't save that note: ${validation.error}` };
            ...
            if (isSemanticSearchConfigured) {
                isAiEnabledForAccount(accountId).then((enabled) => {
                    if (enabled)
                        embedNoteInBackground(accountId, result.data.id).catch(() => { });
                }).catch(() => { });
            }
        ...
        if (lower === "notes") {
            ...
            if (result.data.length === 0)
                return { kind: "text", text: "You haven't saved any notes yet. Try: note <title> | <body>" };
            return {
                kind: "text",
        ...
        if (lower.startsWith("timezone ")) {
            ...
            if (!validation.ok)
                return { kind: "text", text: `⚠ ${validation.error}` };
        ...
        if (lower.startsWith("task ")) {
            ...
            if (!validation.ok)
                return { kind: "text", text: `Hmm, couldn't save that task: ${validation.error}` };
        ...
        if (lower === "tasks") {
            ...
            if (!result.ok)
                return { kind: "text", text: `⚠ ${result.error}` };
            if (result.data.length === 0)
                return { kind: "text", text: "No open tasks — you're all caught up! 🎉" };
        ...
        if (lower.startsWith("done ")) {
            ...
            if (!idPrefix)
                return { kind: "text", text: "You'll need the task id. Try: done a1b2c3d4 — use 'tasks' to see the list." };
            // We only stored an 8-char prefix in the UI; resolve via prefix match.
            const openTasks = await listOpenTasks(accountId, 50);
            if (!openTasks.ok)
                return { kind: "text", text: `⚠ ${openTasks.error}` };
            const match = openTasks.data.find((t) => t.id.startsWith(idPrefix));
            if (!match)
                return { kind: "text", text: "Hmm, couldn't find an open task with that id. Use 'tasks' to see them all." };
        ...
        if (lower.startsWith("remind me ")) {
            ...
            if (!validation.ok)
                return { kind: "text", text: `Hmm, couldn't schedule that: ${validation.error}` };
        ...
        if (lower.startsWith("acknowledge ")) {
            ...
            if (!idPrefix)
                return { kind: "text", text: "You'll need the alarm id. Try: acknowledge a1b2c3d4" };
            const pending = await listPendingReminders(accountId, 50);
            if (!pending.ok)
                return { kind: "text", text: `⚠ ${pending.error}` };
            const match = pending.data.find((r) => r.id.startsWith(idPrefix));
            if (!match)
                return { kind: "text", text: "Hmm, I don't see an alarm with that id." };
        ...
        if (lower === "reminders") {
            ...
            if (!result.ok)
                return { kind: "text", text: `⚠ ${result.error}` };
            if (result.data.length === 0)
                return { kind: "text", text: "No pending reminders right now." };
            return {
                kind: "text",
                text: result.data
        ...
        if (lower.startsWith("snooze ")) {
            ...
            if (!match)
                return { kind: "text", text: "Hmm, I don't see a reminder with that id." };
        ...
        if (lower === "undo") {
            ...
            if (!action)
                return { kind: "text", text: "Nothing to undo right now. Create a note, task, or reminder first." };
            switch (action.kind) {
                case "delete_note": {
        ...
        if (lower.startsWith("connect ")) {
            ...
            if (!validation.ok)
                return { kind: "text", text: `⚠ ${validation.error}` };
            const result = await consumeLinkCode(validation.data.code, validation.data.platform, validation.data.platformUserId, validation.data.displayName);
            if (!result.ok)
                return { kind: "text", text: `⚠ ${result.error}` };
        ...
        if (lower === "notion connect") {
            ...
            if (!stateResult.ok)
                return { kind: "text", text: `⚠ ${stateResult.error}` };
        ...
        if (lower.startsWith("notion database ")) {
            ...
            if (!databaseId)
                return { kind: "text", text: "Try: notion database <database id>" };
            const connectionResult = await getNotionConnection(accountId);
            if (!connectionResult.ok)
                return { kind: "text", text: `⚠ ${connectionResult.error}` };
        ...
        if (lower.startsWith("ask ")) {
            ...
            if (!question)
                return { kind: "text", text: "Try: ask what did I write about the budget?" };
            if (!isGroqConfigured)
                return { kind: "text", text: "Sorry, the AI features haven't been set up yet." };
            ...
            if (aiEnabled) {
                ...
                if (aiResult.ok) {
                    ...
                    if (recognizedIntents.length > 0) {
                        const chatIntent = recognizedIntents.find((i) => i.type === "chat");
 ...
 */
async function executeAiIntent(accountId, intent) {
    switch (intent.type) {
        case "create_note": {
            const validation = safeValidate(createNoteSchema, {
                title: intent.title,
            ...
            if (!validation.ok)
                return `Couldn't save that note: ${validation.error}`;
            const result = await createNote(accountId, validation.data);
            if (!result.ok)
                return `⚠ ${result.error}`;
            ...
            if (isSemanticSearchConfigured)
                void embedNoteInBackground(accountId, result.data.id);
        ...
        case "create_task": {
            ...
            if (!validation.ok)
                return `Couldn't save that task: ${validation.error}`;
            const result = await createTask(accountId, validation.data);
            if (!result.ok)
                return `⚠ ${result.error}`;
        ...
        case "create_reminder": {
            ...
            if (!validation.ok)
                return `Couldn't schedule that: ${validation.error}`;
            const result = await createReminder(accountId, validation.data);
            if (!result.ok)
                return `⚠ ${result.error}`;
        ...
        case "create_alarm": {
            ...
            if (!validation.ok)
                return `Couldn't set that alarm: ${validation.error}`;
            const result = await createReminder(accountId, validation.data);
            if (!result.ok)
                return `⚠ ${result.error}`;
