...
export type UndoAction =
  ...
  | { kind: "delete_reminder"; reminderId: string }
  | { kind: "uncomplete_task"; taskId: string }
  | { kind: "restore_reminder_time"; reminderId: string; previousRemindAt: string };
...
const lastActionByAccount = new Map<string, StoredUndo>();

export function recordUndoableAction(accountId: string, action: UndoAction): void {
...
export function takeUndoableAction(accountId: string): UndoAction | null {
  const stored = lastActionByAccount.get(accountId);
  if (!stored) return null;

  lastActionByAccount.delete(accountId);

  if (Date.now() - stored.recordedAt > UNDO_TTL_MS) return null;
