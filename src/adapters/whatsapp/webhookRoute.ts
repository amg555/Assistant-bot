    ...
    changes?: Array<{
      value?: {
        messages?: Array<{
          from: string;
          text?: { body: string };
          // WhatsApp Cloud API represents both voice notes and regular
          // audio attachments as an "audio" object with a media id.
          audio?: { id: string };
        }>;
...
whatsappRouter.get("/webhook", (req, res) => {
  if (!isWhatsAppConfigured) return res.status(503).send("whatsapp_adapter_disabled");
...
whatsappRouter.post("/webhook", verifyWhatsAppWebhook, async (req, res) => {
  ...
  if (!message || (!message.text?.body && !audioMediaId)) return;
  ...
  try {
    ...
    let inputText: string | undefined = message.text?.body;
    ...
    if (!delivered) logger.warn({ context: "whatsappRouter" }, "message_delivery_failed");
  } catch (err) {
